...
```console
sudo sh -c '(echo "#!/usr/bin/env sh" && curl -L https://github.com/globalbioticinteractions/nomer/releases/download/0.2.10/nomer.jar) > /usr/local/bin/nomer && chmod +x /usr/local/bin/nomer' && nomer clean && nomer version
```
